...
import { SLIDE_DURATION_FRAMES } from './constants.js'
import { SlideVisual } from './visuals/SlideVisual.jsx'

function CaptionBar({ slide, repo, prNumber, frame }) {
...
function SlideContent({ slide, repo, prNumber, frame }) {
  ...
  if (visual.type === 'hero' || visual.type === 'summary') {
    const heroEnter = spring({ frame, fps, config: { damping: 16, stiffness: 80 } })
    return (
      <AbsoluteFill
        style={{
          opacity,
      ...
      >
        <div style={{ opacity: heroEnter, transform: `translateY(${interpolate(heroEnter, [0, 1], [30, 0])}px)` }}>
          <SlideVisual slide={slide} repo={repo} size="lg" frame={frame} durationFrames={duration} animated />
        </div>
      </AbsoluteFill>
    )
  }
  ...
  return (
    <AbsoluteFill style={{ opacity, background: '#0b0c10' }}>
      ...
      <CaptionBar slide={slide} repo={repo} prNumber={prNumber} frame={frame} />
    </AbsoluteFill>
  )
}
